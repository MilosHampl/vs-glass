 ...
 *        [--addons glass/glass-wallpaper.css,glass/tints/glass-tint-blue.css]   (extra CSS injected after glass.css)
 *        [--alpha]   (capture with a transparent page background — for transparent-window evidence; composite later)
 *        [--out DIR] [--suffix -transparent]   (output directory / file-name suffix)
 *
 * The window must be VISIBLE (not occluded): Chromium pauses requestAnimationFrame in occluded windows, so Monaco
 * never renders new lines and captures go stale. Launch the instance with --disable-backgrounding-occluded-windows
 * --disable-renderer-backgrounding to lift that, or keep the window in front.
 *
...
const addons = (opt('--addons', '') || '').split(',').filter(Boolean);
const alphaCapture = args.includes('--alpha');
...
const suffix = opt('--suffix', ''); // appended to every file name, e.g. "-transparent"
...
// rAF must be running (visible window) — otherwise every scene is stale
{
  const r = await send('Runtime.evaluate', { expression: 'new Promise(res => { let done = false; requestAnimationFrame(() => { done = true; res({ raf: true, vis: document.visibilityState }); }); setTimeout(() => !done && res({ raf: false, vis: document.visibilityState }), 1500); })', awaitPromise: true, returnByValue: true });
  const v = r.result.value;
  if (!v.raf) { console.error(`window is occluded (visibilityState=${v.vis}, rAF paused) — bring it to the front or relaunch with --disable-backgrounding-occluded-windows`); process.exit(2); }
}
if (alphaCapture) await send('Emulation.setDefaultBackgroundColorOverride', { color: { r: 0, g: 0, b: 0, a: 0 } });
...
// querySelector that also looks inside VS Code's shadow roots (custom context menus render in a `.shadow-root-host`)
const DEEP_QUERY = `(sel) => { const all = [...document.querySelectorAll(sel), ...[...document.querySelectorAll('.shadow-root-host')].flatMap(h => h.shadowRoot ? [...h.shadowRoot.querySelectorAll(sel)] : [])]; return all.find(e => e.getBoundingClientRect().width > 0) || all[0] || null; }`;
const rect = async (sel) => evalJs(`(() => { const e = (${DEEP_QUERY})(${JSON.stringify(sel)}); if (!e) return null; const r = e.getBoundingClientRect(); return { x: r.x, y: r.y, w: r.width, h: r.height, cx: r.x + r.width / 2, cy: r.y + r.height / 2 }; })()`);
const shot = async (file, clipSel, pad = 24) => {
  const full = path.join(outDir, suffix ? file.replace(/\.png$/, `${suffix}.png`) : file);
...
const openFile = async (name) => {
  ...
  await key('escape'); await sleep(100); await key('meta+p');
  let open = false; for (let i = 0; i < 12 && !open; i++) { await sleep(150); open = await quickInputOpen(); } // poll instead of a second ⌘P (which toggles the help state)
  if (!open) { await key('meta+p'); for (let i = 0; i < 12 && !open; i++) { await sleep(150); open = await quickInputOpen(); } }
  if (!open) throw new Error('quick open did not open');
  ...
  for (let i = 0; i < 10 && await quickInputOpen(); i++) { await key('escape'); await sleep(150); } // never leave quick open behind
  await key('meta+up'); // scroll to the top so the target tokens are in view; Monaco re-renders the viewport on the next frames
  for (let i = 0; i < 20; i++) { await sleep(150); if ((await evalJs(`document.querySelectorAll('.monaco-editor .view-lines .view-line').length`)) > 3) break; }
...
const setLayer2 = async (on) => {
  const css = [fs.readFileSync('glass/glass.css', 'utf8'), ...addons.map(a => fs.readFileSync(a, 'utf8'))].join('\n');
...
const showSidebarView = async () => { await key('escape'); await sleep(100); await key('meta+shift+e'); await sleep(600); }; // ⌘⇧E: deterministic, unlike fuzzy palette matching
...
const SCENES = {
  ...
  hover: async (tag) => {
    ...
    let hov = null;
    for (let attempt = 0; attempt < 3 && !(hov && hov.w >= 10); attempt++) { // hovers are timing-sensitive: park the pointer away, come back, wait
      await mouse(pos.x + 200, pos.y + 200); await sleep(300); await mouse(pos.x, pos.y); await sleep(1600 + attempt * 600);
      hov = await rect('.monaco-editor .monaco-hover');
    }
    if (!hov || hov.w < 10) throw new Error('hover did not appear');
    await shot(`${tag}-hover.png`, '.monaco-editor .monaco-hover', 60); await mouse(pos.x, pos.y + 300);
  },
  suggest: async (tag) => {
    await openFile('glass.ts');
    const pos = await evalJs(`(() => { const lines = [...document.querySelectorAll('.monaco-editor .view-lines .view-line')].filter(l => l.textContent.trim().length > 6); const l = lines[Math.floor(lines.length / 2)]; if (!l) return null; const r = l.getBoundingClientRect(); return { x: r.x + Math.min(r.width, 260), y: r.y + r.height / 2 }; })()`);
  ...
  menu: async (tag) => {
    // needs "window.menuStyle": "custom" in the profile (native menus live outside the DOM); right-click first, fall back to the command
    // .view-lines can be taller than the viewport (its centre may be off-screen), so click near its top-left instead
    await openFile('glass.ts'); const v = await rect('.monaco-editor .view-lines'); const r = { cx: v.x + 220, cy: v.y + 120 }; await mouse(r.cx, r.cy, 'click'); await sleep(200);
    await mouse(r.cx, r.cy, 'click', 'right'); await sleep(700);
    let m = await rect('.monaco-menu-container');
    if (process.env.VSG_DEBUG) console.log('    menu after right-click:', JSON.stringify(m), await evalJs(`JSON.stringify([...document.querySelectorAll('.shadow-root-host')].map(h => h.shadowRoot ? [...h.shadowRoot.querySelectorAll('.monaco-menu-container, .monaco-menu')].map(e => e.className.toString().slice(0, 40) + ' ' + Math.round(e.getBoundingClientRect().width)) : 'no-shadow'))`), 'click at', r.cx, r.cy);
    if (!m || m.w < 10) { await key('shift+f10'); await sleep(700); m = await rect('.monaco-menu-container'); }
    if (!m || m.w < 10) throw new Error('context menu did not open (is window.menuStyle "custom"?)');
    await shot(`${tag}-menu.png`, '.monaco-menu-container', 60); await closeAll();
  },
